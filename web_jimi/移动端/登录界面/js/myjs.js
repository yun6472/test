  ...
  if(calln.length>1){
	  ...
	  for(var t=0;t<calln.length;t++)
	  {
		var borderBottom = '';
			if(t != calln.length -1 )
			{
				borderBottom = 'style="border-bottom:0.1em solid #dddddd;"';
			}
			telinfospan=telinfospan+'<div class="m3 bj-w2 tc p1"  '+borderBottom+' onclick=gettel("'+calln[t]+'")>'+parseInt(t+1)+'、'+calln[t]+'</div>'; 
	  }
	  alertawg2(telinfospan,1,"","请选择拨打号码")
  }else{
...
function cztsalert(txt){  
  ...
  alertdiv.className="tbox pos2 boxu boxu-s";//tmbj-b2
  ...
  <div class="tscsdiv boxu boxu-h m4">\
    <div class="boxbf1"></div>\
	<div class="tmbj-b7 bk-b0 p3 tcol-w1 ts-0d8" style="padding:.4em 1em;box-sizing:border-box;line-height:3em;border-radius:.5em;"><span style="font-size:1.2em;">'+txt+'</span></div>\
	<div class="boxbf1"></div>\
  </div>\
  <div class="boxbf1"></div>\
  '
  ...
  //loadingDialog.className="tbox pos2 boxu boxu-s";//tmbj-b2
  //loadingDialog.style.zIndex="0";
  //loadingDialog.style.left="0px";
 // loadingDialog.style.top="0px";
  //divmargin tc 
  loadingDialog.innerHTML = '<div id="loading_dialog" class="" style="display:block;"><dt class="myloadimg" style="width:2em;height:2em;"></dt><dt class="p2">加载中..</dt></div>';
...
{
	var  bodydiv = document.body;
   bodydiv.appendChild(loadingDialog);
...
{
	//dialog.display = "none";
	// bodydiv.removeChild(loadingDialog);
	 var bodydiv = document.body;
	   setTimeout(function(){
	   //alert("SSSS");
					bodydiv.removeChild(loadingDialog);
  ...
  <div class="tbox" style="background:rgba(0,0,0,0.5);">\
   <div class="myasktbox pos1">\
    <div class="divmargin yj2 bj-w6 bks2">\
      <p class="divmargin p3 pt4 pb4 bj-w8 tb1 tcol-w1">提示</p>\
...
var newalert2;
function alertawg2(txt,act,fun,title){ 
  var askact=act||0,laskbtn="";
  if(askact==2){
    var laskbtn='\
	<div class="divmargin p3 bj-w2 tcol-w9 tc"><btn class="p2 ml2 mr2 tc bj-w8 tcol-w1 ona sbbj3-4">确定</btn></div>\
  ...
  var fdsagewal = document.body;
  newalert2 = document.createElement("div");
  newalert2.className="tbox pos2 askdis-not";
  newalert2.style.zIndex="3000";
  newalert2.innerHTML='\
  <div class="tbox" style="background:rgba(0,0,0,0.5);">\
   <div class="myasktbox pos1">\
    <div class="divmargin yj2 bj-w6 bks2">\
      <p class="divmargin p3 pt4 pb4 bj-w8 tb1 tcol-w1">'+title+'</p>\
      <div class="divmargin p3 pt4 pb4 bj-w2 tcol-w9">'+txt+'</div>\
	  '+laskbtn+'\
	</div>\
   </div>\
  ...
  ';
  fdsagewal.appendChild(newalert2);
  setTimeout(function(){
    if(newalert2) newalert2.className=newalert2.className.replace("askdis-not","askdis-yes");
	if(!askact){
	  setTimeout(function(){
        if(newalert2) newalert2.className=newalert2.className.replace("askdis-yes","askdis-not");
	    setTimeout(function(){
        if(newalert2) newalert2.parentNode.removeChild(newalert2);
        },600);
       },5000);
	 }
	 
	 if(askact==3){
	   var btnok=newalert2.getElementsByTagName("btn");
	   if(btnok[0]) btnok[0].addEventListener('click',function(){
															   newalert2.parentNode.removeChild(newalert2);
															   if(fun) return fun(1);
															   },false);
	   if(btnok[1]) btnok[1].addEventListener('click',function(){
															   newalert2.parentNode.removeChild(newalert2);
	 ...
	 if(newalert2){
	     newalert2.addEventListener('click', function(){
         if(newalert2) newalert2.className=newalert2.className.replace("askdis-yes","askdis-not");
	     setTimeout(function(){
         if(newalert2) newalert2.parentNode.removeChild(newalert2);
		 if(fun) return fun();
